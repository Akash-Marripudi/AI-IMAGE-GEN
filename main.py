...
prompt_label=ctk.CTkLabel(left_frame,text="Prompt")
prompt_label.grid(row=0,column=0,padx=10,pady=10)

prompt_entry=ctk.CTkTextbox(left_frame,height=10)
prompt_entry.grid(row=0,column=1,padx=10,pady=0)

style_label=ctk.CTkLabel(left_frame,text="style")
style_label.grid(row=1,column=0,padx=10,pady=10)
style_dropdown=ctk.CTkComboBox(left_frame,values=["Realistic","Cartoon","3D Art","Flat Art"])
style_dropdown.grid(row=1,column=1,padx=10,pady=10)

number_label=ctk.CTkLabel(left_frame,text="# Images")
number_label.grid(row=2,column=0)
number_slider=ctk.CTkSlider(left_frame,from_=1,to=10,number_of_steps=9)
number_slider.grid(row=2,column=1)

generate_butoon=ctk.CTkButton(left_frame,text="Generate",command=generate)
generate_butoon.grid(row=3,column=0,columnspan=2,sticky="news",padx=10,pady=10)
